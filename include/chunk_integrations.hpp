...
public:
    explicit PostgresConnection(const std::string& conn_string) : connection_string(conn_string) {}
...
public:
    explicit MongoConnection(const std::string& mongodb_uri) : uri(mongodb_uri) {}
...
public:
    DatabaseChunkStore(std::unique_ptr<DatabaseConnection> conn,
        ...
        : connection(std::move(conn)), table_name(table) {}
    ...
    void store_chunks_postgres(const std::vector<std::vector<U>>& chunks) {
        ...
        try {
            ...
            for (size_t i = 0; i < chunks.size(); ++i) {
                ...
                txn.exec_params("INSERT INTO " + table_name +
                                    " (chunk_id, chunk_data, metadata) VALUES ($1, $2, $3)",
                                i, chunk_json, chunk_json);
            }
    ...
    void store_chunks_mongodb(const std::vector<std::vector<U>>& chunks) {
        ...
        try {
            ...
            for (size_t i = 0; i < chunks.size(); ++i) {
                auto chunk_json = serialize_chunk(chunks[i], i);
                documents.push_back(bsoncxx::from_json(chunk_json));
            }
...
public:
    explicit KafkaConnection(const std::string& broker_uri) : broker(broker_uri) {}
...
public:
    explicit RabbitMQConnection(const std::string& uri_string) : uri(uri_string) {}
    ...
    bool connect() override {
        try {
            ...
            amqp_login(conn, info.vhost, 0, 131072, 0, AMQP_SASL_METHOD_PLAIN, info.user,
                       info.password);
...
public:
    ChunkMessageQueue(std::unique_ptr<MessageQueueConnection> conn, const std::string& topic_name)
        : connection(std::move(conn)), topic(topic_name) {}
        ...
        try {
            for (size_t i = 0; i < chunks.size(); ++i) {
                ...
                kafka->get_producer()->produce(
                    cppkafka::MessageBuilder(topic).payload(message).key(std::to_string(i)));
            }
    ...
    void publish_chunks_rabbitmq(const std::vector<std::vector<T>>& chunks) {
        ...
        try {
            ...
            for (size_t i = 0; i < chunks.size(); ++i) {
                ...
                amqp_basic_publish(rabbitmq->get_connection(), 1, amqp_cstring_bytes(""),
                                   amqp_cstring_bytes(topic.c_str()), 0, 0, nullptr,
