...
private:
    ...
    static T calculate_centroid(const std::vector<T>& chunk) {
        ...
        return std::accumulate(chunk.begin(), chunk.end(), T{}) / static_cast<T>(chunk.size());
    }
...
public:
    ...
    SizeMetrics compute_size_metrics(const std::vector<std::vector<T>>& chunks) {
        ...
        return {mean, variance, *std::min_element(sizes.begin(), sizes.end()),
                *std::max_element(sizes.begin(), sizes.end()), entropy};
    }
    ...
    double compute_quality_score(const std::vector<std::vector<T>>& chunks) {
        ...
        return (avg_cohesion * cohesion_weight) + (separation * separation_weight) +
