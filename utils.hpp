...
template <typename T> class ChunkManipulator {
public:
  static std::vector<std::vector<T>>
  merge_chunks(const std::vector<std::vector<T>> &chunks1,
  ...
  static std::vector<std::vector<T>>
  filter_chunks(const std::vector<std::vector<T>> &chunks,
                std::function<bool(const std::vector<T> &)> predicate) {
    std::vector<std::vector<T>> result;
    std::copy_if(chunks.begin(), chunks.end(), std::back_inserter(result),
                 predicate);
    return result;
  }
  ...
  static std::vector<std::vector<T>> transform_chunks(
    ...
    std::vector<std::vector<T>> result;
    std::transform(chunks.begin(), chunks.end(), std::back_inserter(result),
                   transformer);
    return result;
  }
};
...
template <typename T> class ChunkGenerator {
private:
  static std::random_device rd;
  static std::mt19937 gen;

public:
  static std::vector<T> generate_random_data(size_t size, T min_val,
                                             T max_val) {
  ...
  static std::vector<std::vector<T>> generate_random_chunks(size_t num_chunks,
                                                            size_t chunk_size,
                                                            T min_val,
                                                            T max_val) {
...
template <typename T> std::random_device ChunkGenerator<T>::rd;
template <typename T>
